  ...
  <div class="min-h-[80vh] flex items-center justify-center px-4 relative">
    ...
    <div class="max-w-md w-full bg-slate-950/60 border border-white/10 rounded-3xl p-8 backdrop-blur-md shadow-2xl relative overflow-hidden z-10">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-extrabold text-white tracking-tight">Crea il tuo Account</h1>
        <p class="text-xs text-gray-400 mt-1">Inizia subito a organizzare la tua watchlist gratuita.</p>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Nome Utente</label>
          <input
            v-model="username"
            type="text"
      ...
      <p class="text-xs text-gray-500 text-center mt-6">
        Hai già un account?
        <NuxtLink to="/login" class="text-purple-400 font-semibold hover:underline">Accedi</NuxtLink>
      </p>
    </div>
  </div>
</template>
