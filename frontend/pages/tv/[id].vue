        ...
        <div class="absolute bottom-0 left-0 w-full p-6 md:p-12 lg:p-16 flex flex-col md:flex-row items-start md:items-end gap-8 max-w-7xl mx-auto z-10">
          ...
          <div class="flex-1 space-y-4 mb-4 md:mb-0 w-full">
             <div class="flex items-center gap-3 mb-2 flex-wrap">
              ...
              <span v-if="item?.score" class="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold text-yellow-500 bg-yellow-500/10 border border-yellow-500/20 backdrop-blur-md rounded">
                <svg class="w-3.5 h-3.5 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                Il mio voto: {{ item.score }}/10
              </span>
              <!-- Badge Ultimo visto -->
              <span
              ...
              <div class="space-y-4 text-sm">
                ...
                <div v-if="item.score">
                  ...
                  <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 font-semibold">
                    ★ {{ item.score }} / 10 <span class="text-xs text-gray-400" v-if="item.liked">({{ likedLabel(item.liked) }})</span>
                  </span>
                </div>
